...
object Q3 {
    def main(args: Array[String]) = {  // this is the entry point to our code
        ...
        saveit(cost, "spark3output") // save the rdd to home directory for gradescope
        runTest(sc) 
    ...
    def doRetail(input: RDD[String]): RDD[(String, Double)] = {
        ...
        val line_costs = line_data.map(line => (line._1, line._2.toDouble*line._3.toDouble)) // get kv with invoiceNo and cost (quanitity * unitprice)
        val total_cost = line_costs.reduceByKey((x,y) => x+y)  // combine all the same orders, will now have each order and total cost of order
        total_cost
    }
    
    def saveit(cost: RDD[(String, Double)], name: String) = {  // simply saves the file
        cost.saveAsTextFile(name)
    }
